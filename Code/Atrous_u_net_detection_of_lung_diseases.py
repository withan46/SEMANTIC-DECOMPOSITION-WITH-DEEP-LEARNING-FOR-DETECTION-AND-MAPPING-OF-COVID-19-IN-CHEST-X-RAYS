# -*- coding: utf-8 -*-
"""fold 2 Atrous U-Net  Detection of lung diseases

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1rw-g_4N5bJs1YRxdVP_NNNTmSgKyl_Rl
"""
...
def prepare_and_split_data(df, test_size=0.1, val_size=0.1):
    train_val_df, test_df = train_test_split(df, test_size=test_size, random_state=56, stratify=df['category'])
    adjusted_val_size = val_size / (1 - test_size)
    train_df, val_df = train_test_split(train_val_df, test_size=adjusted_val_size, random_state=56, stratify=train_val_df['category'])
    return train_df, val_df, test_df


# Modified U-Net Model Architecture with Atrous Convolutions
def atrous_unet_model(input_size=(256, 256, 3)):
    ...
    # Dilated convolutions in the deeper layers
    c3 = Conv2D(256, (3, 3), activation='relu', padding='same', dilation_rate=2)(p2)
    c3 = Conv2D(256, (3, 3), activation='relu', padding='same', dilation_rate=2)(c3)
    p3 = MaxPooling2D((2, 2))(c3)

    c4 = Conv2D(512, (3, 3), activation='relu', padding='same', dilation_rate=4)(p3)
    c4 = Conv2D(512, (3, 3), activation='relu', padding='same', dilation_rate=4)(c4)
    p4 = MaxPooling2D((2, 2))(c4)

    # Bottleneck
    c5 = Conv2D(1024, (3, 3), activation='relu', padding='same', dilation_rate=8)(p4)
    c5 = Conv2D(1024, (3, 3), activation='relu', padding='same', dilation_rate=8)(c5)
...
# Set the path for the checkpoint
checkpoint_path = "/content/drive/My Drive/Colab Notebooks/model_checkpoint.h5"

# Ensure model loss matches the input shape
atrous_unet = atrous_unet_model(input_size=(256, 256, 3))
atrous_unet.compile(optimizer='adam', loss='binary_crossentropy', metrics=[dice_coefficient])
atrous_unet.summary()

# Create checkpoint callback for Atrous U-Net
checkpoint_callback_atrous = ModelCheckpoint(
    checkpoint_path,
...
# Train the Atrous U-Net model
history_atrous = atrous_unet.fit(train_gen, validation_data=val_gen, epochs=14, verbose=1, callbacks=[checkpoint_callback_atrous, early_stop])
...
# Calculate confusion matrices for Atrous U-Net
train_cm_atrous = batch_confusion_matrix(train_gen, atrous_unet)
val_cm_atrous = batch_confusion_matrix(val_gen, atrous_unet)
test_cm_atrous = batch_confusion_matrix(test_gen, atrous_unet)

# Evaluate on test data for Atrous U-Net
test_loss_atrous, test_accuracy_atrous = atrous_unet.evaluate(test_gen, verbose=0)
print(f"Test Loss (Atrous U-Net): {test_loss_atrous}, Test Accuracy (Atrous U-Net): {test_accuracy_atrous}")

print("Train Set Confusion Matrix (Atrous U-Net):")
print(train_cm_atrous)

print("Validation Set Confusion Matrix (Atrous U-Net):")
print(val_cm_atrous)

print("Test Set Confusion Matrix (Atrous U-Net):")
print(test_cm_atrous)

# clear_memory()

# Print confusion matrices and metrics for each set for Atrous U-Net
for name, cm in [('Train', train_cm_atrous), ('Validation', val_cm_atrous), ('Test', test_cm_atrous)]:
    precision, recall, f1, accuracy = calculate_metrics_from_cm(cm)
    print(f"{name} Set Confusion Matrix (Atrous U-Net):")
    print(cm)
    print(f"{name} Set Metrics (Atrous U-Net): Precision: {precision:.4f}, Recall: {recall:.4f}, F1 Score: {f1:.4f}, Accuracy: {accuracy:.4f}\n")

# Plot segmentation training history for Atrous U-Net
plot_segmentation_training(history_atrous)
...
visualize_predictions(val_gen, atrous_unet, num_samples=3)
...
# Call to visualize predictions with overlay on validation data for Atrous U-Net
visualize_predictions_with_overlay(val_gen, atrous_unet, num_samples=3)

tf.keras.utils.plot_model(atrous_unet, show_shapes=True)

# Define model name for the experiment results
model_name_atrous = "Atrous U-Net Model"

# Initialize a list to store results for Atrous U-Net
results_atrous = []

# Iterate through each set and collect results for Atrous U-Net
for set_name, cm in [('Train', train_cm_atrous), ('Validation', val_cm_atrous), ('Test', test_cm_atrous)]:
    precision, recall, f1, accuracy = calculate_metrics_from_cm(cm)
    result = {
        'Experiment ID': "2",
        'Model': model_name_atrous,
        'Set': set_name,
        'Accuracy': accuracy,
        'Precision': precision,
        'Recall': recall,
        'F1 Score': f1,
        'Timer(minutes)': training_time
    }
    results_atrous.append(result)

# Convert results to DataFrame for Atrous U-Net
results_df_atrous = pd.DataFrame(results_atrous)

# Export to Excel
results_df_atrous.to_excel('experiment_results.xlsx', index=False)
